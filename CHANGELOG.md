...
README 的定位改成「**只回答四件事**」：能帮什么忙 / 怎么装 / 怎么用 / 详细内容去哪看。
英文版 14,790 → **5,447 字节（-63%）**，中文版 14,753 → **5,214（-65%）**。

搬走的内容（**是搬，不是删** —— 全部能在深度文档里找到）：

| 搬走的内容 | 去向 |
|---|---|
| 「失败模式 → 对策」13 行对照表 | `CONCEPTS.md` **附 A**（§1 讲来龙去脉，此表只做速查） |
| 「内容库」11 行规模清单 + 13 类输入源预案 | `TUTORIAL.md` **附：内容库规模清单** |
| 阶段门全表 / 8 条命令全表 | 本就在 `TUTORIAL.md` §2 走查与 §3 命令手册 |
| 完整目录树 | `references/filemap.md`（已有逐文件职责，比树更全） |
| F16 / F17–F20 的两条判据教训 | 本就在 `CONCEPTS.md` §7.1（写得比 README 更完整） |

同时补上 skills.sh 的一行安装命令 `npx skills add XanthanL/website-style-router` ——
README 讲「如何安装」，就该给最短的那条路径。

**判断标准**：README 里每一行都要回答「读者现在就需要知道吗」。
答「以后可能会用到」的一律外移 —— 那是深度文档的活。
